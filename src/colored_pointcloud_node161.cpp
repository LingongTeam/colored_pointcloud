...
class RsCamFusion
{
  ...
  public:
    ...
    // sensor_msgs to cv image   
      ...
      pcl::PointCloud<PointXYZIRT>::Ptr input_cloud_ptr(new pcl::PointCloud<PointXYZIRT>);
      ...
      for(int i=0;i<imagePoints.size();i++)
      {
        ...
        {
          cv::circle(image_to_show, imagePoints[i], 1, dis_color[i], 2, 8, 0);            //void cv::circle (InputOutputArray img, Point center, int radius, const Scalar &color, int thickness=1, int lineType=LINE_8, int shift=0)
          ...
          PointXYZRGBI point;                                                             //相机范围外的点云，全部涂白色
      ...
      {  
        ...
        // }
        publishCloudtoShow(colored_cloud_showpub, cloud_header, colored_cloud_transback);
      } 
    ...
    void saveData(const std_msgs::Header& image_header, const cv::Mat image, const std_msgs::Header& cloud_header,
                      const pcl::PointCloud<PointXYZIRT>::ConstPtr& cloud, pcl::PointCloud<PointXYZRGBI>::Ptr colored_cloud)
    {
